...
class RelationNet(MetaTemplate):
    def __init__(self, model_func, n_way, n_support, loss_type='mse'):
        ...
        self.traditional_cls_loss = nn.CrossEntropyLoss()
    ...
    def set_forward(self, x, is_feature=False):
        ...
        return relations, traditional_cls_score
    ...
    def set_forward_adaptation(self, x, is_feature=True):  # overwrite parent function
        ...
        for epoch in range(100):
            ...
            scores, traditional_cls_score = self.set_forward(sub_x, is_feature=True)
            if self.loss_type == 'mse':
    ...
    def set_forward_loss(self, x, label):
        ...
        if self.loss_type == 'mse':
            ...
            meta_loss = self.loss_fn(scores, y_oh)
        else:
            y = Variable(y.cuda())
            meta_loss = self.loss_fn(scores, y)
        if self.training:
            traditional_cls_loss = self.traditional_cls_loss(traditional_cls_score, label)
        else:
            traditional_cls_loss = 0
