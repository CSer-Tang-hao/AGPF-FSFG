...
class MetaTemplate(nn.Module):
    ...
    def forward(self, x):
        out2, out3, out4, out = self.feature.forward(x)
        return out
    ...
    def parse_feature(self, x, is_feature):
        ...
        if is_feature:
            ...
            return z_support, z_query, None
        else:
            x = x.contiguous().view(self.n_way * (self.n_support + self.n_query), *x.size()[2:])
            z_all, traditional_cls_score = self.feature.forward(x)
            new_shape = [self.n_way, self.n_support + self.n_query]
            new_shape.extend(z_all.shape[1:])
            z_all = z_all.view(new_shape)
            z_support = z_all[:, :self.n_support]
            z_query = z_all[:, self.n_support:]

            return z_support, z_query, traditional_cls_score
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        for i, (x, label) in enumerate(train_loader):
            ...
            if self.change_way:
                self.n_way = x.size(0)
            optimizer.zero_grad()
            loss = self.set_forward_loss(x, label)
            loss.backward()
            optimizer.step()
